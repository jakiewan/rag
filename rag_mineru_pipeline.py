"""
rag_mineru_pipeline.py
======================

RAG 流水线整合脚本 —— 将本目录（个人/0914work/rag）下的 RAG 工程串联起来。

整合来源（同级仓库中的 01-07 既有脚本）：
    01-document_loader.ipynb  —— 多源文档加载（Text / Markdown / PDF / Word / Web）
    02-minerU-base.ipynb      —— MinerU 解析 PDF（命令行模式）
    03-test.py                —— 04 中 mineru 命令的最简子进程调用
    04-mineru-pdf.py          —— mineru 子进程 + 日志封装
    05-image-handler.py       —— OpenAI 兼容 VLM（qwen-vl-plus）生成图片中文标题
    06-upload_image.py        —— MinIO fput_object 上传单张图片
    07-md_im.py               —— 整条流水线：mineru → 读 MD → 扫描引用 → VLM 摘要 → MinIO 上传 → 链接替换

本文件相比 01-07 的关键修改：
    1. 路径不再写死 Windows 反斜杠，全部改为基于本文件位置的 Path(__file__).parent 自适应，
       兼容 macOS / Linux / Windows。
    2. 阿里云百炼 API key 改为从环境变量 / .env 读取（不再硬编码在源码中）。
    3. 三大外部依赖都设计为"缺失时降级"：mineru CLI 没装就跳过 PDF 解析、MinIO 没启就
       写 mock URL、VLM 凭据缺失就用占位描述。保证脚本在最小环境也能跑通主链路。
    4. argparse 提供 --mode / --pdf / --md / --skip-* 等开关，便于复现与运行截图。

运行示例：
    # 最小依赖轻量验证（无 mineru / MinIO / VLM 也能跑）
    python3 rag_mineru_pipeline.py --mode mock

    # 完整链路：复用同级 result/sample/auto/sample.md 作为 MinerU 解析产物
    python3 rag_mineru_pipeline.py --mode live --md ../result/sample/auto/sample.md

    # 从 PDF 一路跑到 MinIO
    python3 rag_mineru_pipeline.py --mode live --pdf ./knowledge_base/sample.pdf
"""
...
class Config:
    ...
    dashscope_api_key: str = os.getenv("DASHSCOPE_API_KEY", "")        # API key（不提供则跳过 VLM）
...
def step1_mineru_parse(pdf_path: Path, output_dir: Path, cfg: Config) -> Optional[Path]:
    ...
    logger.info("[Step1] MinerU 解析完成: %s", md_path)
    return md_path
...
def step2_read_md(md_path: Path) -> Tuple[str, Path, Path]:
    ...
    logger.info("[Step2] MD 长度=%d 字符, images_dir=%s", len(md_content), images_dir)
...
# ---------------------------------------------------------------------------
def _find_image_contexts(md_content: str, image_filename: str,
                         max_chars: int = 100) -> List[Tuple[str, str, str]]:
    """
    ...
    for idx, line in enumerate(lines):                                  # 遍历行
        ...
        pre = _extract_paragraphs(lines[heading_idx + 1: idx] if heading_idx >= 0 else lines[:idx],
                                  max_chars, direction="backward")        # 上文
        post = _extract_paragraphs(lines[idx + 1: next_idx], max_chars, direction="forward")  # 下文
        out.append((heading, pre, post))                                 # 收集

    return out


def _extract_paragraphs(lines: List[str], max_chars: int, direction: str = "forward") -> str:
    """
    在字符数限制内尽量取完整段落。
...
def step3_scan_images(md_content: str, images_dir: Path) -> List[Tuple[Path, Tuple[str, str, str]]]:
    """
...
def step5_upload_and_replace(targets: List[Tuple[Path, Tuple[str, str, str]]],
                             summaries: Dict[str, str],
                             md_content: str,
                             doc_stem: str,
                             cfg: Config) -> Tuple[str, Dict[str, str]]:
    """
        ...
        if client is not None:                                           # 有客户端
            try:
                client.fput_object(                                      # 上传
                    cfg.minio_bucket, object_name, str(img_path),
    ...
    for fn, s in summaries.items():                                      # 替换链接
        ...
        new_md = re.sub(
            r"!\[(.*?)\]\((.*?" + re.escape(fn) + r".*?)\)",
            f"![{s}]({remote[fn]})", new_md, flags=re.IGNORECASE,
        )
...
def run_pipeline(args: argparse.Namespace) -> int:
    ...
    # 解析输出目录：默认 0914work/rag/result/，避免污染父级
    ...
    new_md, remote = step5_upload_and_replace(
        targets, summaries, md_content, md_path.stem, cfg,
    )
    ...
    logger.info("[Step6] 已写入: %s", out_md)

    summary_path = out_dir / f"{md_path.stem}_summary.json"
    summary_path.write_text(
        json.dumps({"summaries": summaries, "remote_urls": remote},
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--mode", choices=["live", "mock"], default="live",
